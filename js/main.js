  ...
  calcBtn.on('click', (error) => {
    ...
    switch (selectedMethod) {
      ...
      case falsePositionId:
        calcBisectionAndFalsePos(xl.val(), xu.val(), err.val(), maxIter.val())
        break
      case simpleFixedPointId:
        calcSimplePoint(x0.val(), err.val(), maxIter.val())
        break
      case newtonID:
        calcNewton(mathInput.val(), x0.val(), err.val(), maxIter.val(), diffInput.val())
        break
      case gaussID:
        if (isValidSystem()) {
          gauss()
